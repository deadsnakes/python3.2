...
def setup(app):
    # monkey-patch HTMLTranslator to understand refcount nodes; patching only
    # HTML is fine since we only build that -- fk 2014-01-16
    def visit_refcount(self, node):
        self.body.append(self.starttag(node, 'em', '', CLASS='refcount'))
    def depart_refcount(self, node):
        self.body.append('</em>')
    from sphinx.writers.html import HTMLTranslator
    HTMLTranslator.visit_refcount = visit_refcount
    HTMLTranslator.depart_refcount = depart_refcount
